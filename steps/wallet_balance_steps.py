...
@given('I have wallet balance request payload')
def step_have_wallet_balance_payload_from_table(context):
    """Prepare wallet balance request payload from data table"""
    # Read data from the table in feature file
    payload_data = {}
    for row in context.table:
        payload_data[row['field']] = row['value']
    
    currency = payload_data.get('currency', 'ZWG')
    ...
    # Build payload from table data
    context.wallet_balance_payload = {
        "country": payload_data.get('country', 'ZW'),
        "currency": payload_data.get('currency', 'ZWG'),
        "providerName": payload_data.get('providerName', 'ecocash'),
        "providerCode": payload_data.get('providerCode', 'ecocash'),
        "pin": encrypted_pin,
        "instrumentToken": instrument_token
    }
    logger.info(f"Wallet balance payload prepared from table for currency: {currency}")
    print(f"\n✓ Wallet balance payload prepared for {currency}")
    print(f"  Country: {payload_data.get('country')}")
    print(f"  Provider: {payload_data.get('providerName')}")
...
def step_have_wallet_balance_payload_with_currency(context, currency):
    """Prepare wallet balance request payload with specified currency (backward compatibility)"""
    # Get instrument token from payment options if available
    instrument_token = getattr(context, 'instrument_token', None)
