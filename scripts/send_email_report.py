...
class EmailReportGenerator:
    ...
    def generate_html_report(self, env, tags):
        ...
        content_config = self.config['email'].get('content', {})
        if content_config.get('attach_full_report', False):
            html += f"""
                <div class="info-box">
                    <h3 style="margin-top: 0;">📎 Attached Reports</h3>
                    <p style="margin: 10px 0;">
                        <strong>✅ HTML Test Report</strong> - Full detailed test execution report attached to this email
                    </p>
                    <p style="margin: 10px 0; font-size: 13px; color: #666;">
                        📥 Download the attached HTML file and open it in your browser to view the complete test results
                    </p>
                </div>
            """
        else:
            html += f"""
                <div class="info-box">
                    ...
                    <ul>
                        <li><strong>Allure Report:</strong> reports/allure-report/index.html (Available on CI/CD server)</li>
                        <li><strong>HTML Report:</strong> reports/html-report/report.html (Available on CI/CD server)</li>
                        <li><strong>Log Files:</strong> logs/ (Available on CI/CD server)</li>
                    </ul>
                </div>
            """
    ...
    def send_email(self, env, tags):
        ...
        # Attach HTML report file if configured
        if content_config.get('attach_full_report', False):
            report_path = self.project_root / 'reports' / 'html-report' / 'report.html'
            if report_path.exists():
                try:
                    ...
                    filename = f"test_report_{env}_{timestamp_str}.html"
                    ...
                    print(f"📎 Attached HTML report: {filename}")
                except Exception as e:
                    print(f"⚠️ Failed to attach HTML report: {str(e)}")
            else:
                print(f"⚠️ HTML report not found at: {report_path}")
